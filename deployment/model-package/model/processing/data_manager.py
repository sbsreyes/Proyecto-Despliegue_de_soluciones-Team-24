...
# Cache simple del catálogo de municipios (se carga una vez)
_CATALOGO_CACHE: dict = {}


def get_catalogo_municipio(cod_municipio: str) -> dict:
    """
    Retorna info estática del municipio: altitud, cat_altitud, poblacion,
    NOM_MPIO, NOM_DPTO.

    Se construye lazy desde dataset_dengue_completo.csv y se cachea.
    """
    global _CATALOGO_CACHE

    if not _CATALOGO_CACHE:
        df = load_dataset(file_name=config.app_config.train_data_file)
        # Tomar el registro más reciente del municipio (mejor estimación)
        ...
        _CATALOGO_CACHE = catalogo_df.set_index("cod_municipio").to_dict("index")

    return _CATALOGO_CACHE.get(cod_municipio, {
        "NOM_MPIO": "DESCONOCIDO",
        "NOM_DPTO": "DESCONOCIDO",
        "altitud_msnm": 1000.0,
        "cat_altitud": "Sin dato",
        "poblacion": 50000.0,
    })
